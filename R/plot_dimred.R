...
#' @param n_arrow_bins Number of bins to use for the arrows
#'
...
project_waypoints <- function(
  ...
  # add evenly distributed arrows, by binning the edges in 2D
  waypoint_edges <- waypoint_edges %>%
    rowwise() %>%
    mutate(comp_1_mid = mean(comp_1_from, comp_1_to), comp_2_mid = mean(comp_2_from, comp_2_to)) %>%
    ungroup() %>%
    mutate(bin = paste0(cut(comp_1_mid, 5), cut(comp_2_mid, 5))) %>%
    group_by(bin) %>%
    mutate(arrow = row_number() == 1) %>%
    ungroup()

#   waypoint_edges <- waypoint_edges %>%
#     mutate(arrow = (dplyr::row_number() != 0) & (dplyr::row_number() %% round(n()/n_arrows) == 0))
