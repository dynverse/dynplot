...
linearise_cells <- function(
  ...
  if (no_margin_between_linear) {
    # add margin only if froms not directly connected, or if to is a forking milestone, or if from is a converging milestone
    milestone_network$add_margin <- (milestone_network$to != lead(milestone_network$from, default = "")) |
      ...
      (table(milestone_network$to)[milestone_network$from] > 1)
  } else {
